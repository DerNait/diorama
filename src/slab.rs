...
impl Slab {
    /// Crea un slab desde el **centro de bloque completo** y el tamaño del cubo completo.
    /// El slab ocupará media altura del cubo:
    /// - Bottom: [center.y - half.y, center.y]
    /// - Top:    [center.y, center.y + half.y]
    pub fn from_block_center_size(center: Vector3, cube_size: Vector3, half: SlabHalf, material: Material) -> Self {
        let h = cube_size * 0.5;
        let (min, max) = match half {
            SlabHalf::Bottom => (Vector3::new(center.x - h.x, center.y - h.y, center.z - h.z),
                                 Vector3::new(center.x + h.x, center.y,       center.z + h.z)),
            SlabHalf::Top    => (Vector3::new(center.x - h.x, center.y,       center.z - h.z),
                                 Vector3::new(center.x + h.x, center.y + h.y, center.z + h.z)),
        };
...
/// Igual que en cube.rs, pero con control de v_tex para caras laterales (media textura).
...
impl RayIntersect for Slab {
    fn ray_intersect(&self, ro: &Vector3, rd: &Vector3) -> Intersect {
        ...
        // UV por cara (idéntico a Cube, pero mapearemos v para laterales)
        ...
        // === Ajuste “media textura” SOLO en LATERALES ===
        let is_side = matches!(face, Face::PosX | Face::NegX | Face::PosZ | Face::NegZ);
        if is_side {
            // v va de 0..1 a lo alto del slab. Re-muestreamos solo media franja.
            v = match self.half {
                SlabHalf::Bottom => 0.5 + 0.5 * v, // mitad inferior de la imagen
                SlabHalf::Top    => 0.5 * v,       // mitad superior de la imagen
            };
        }
        // En top/bottom NO se remapea: usan la imagen completa.
